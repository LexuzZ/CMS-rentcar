...
<head>
    ...
    <style>
        * { font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .tipe-btn:hover { border-color:#c2410c; color:#c2410c; }
        .tipe-btn.active {
            background:#fff7ed; border-color:#c2410c; color:#c2410c;
            box-shadow:0 0 0 3px rgba(194,65,12,.1);
        }
        .tipe-btn .tipe-icon {
            width:32px; height:32px; border-radius:8px;
            display:flex; align-items:center; justify-content:center;
            background:#f3f4f6; flex-shrink:0; transition:background .15s;
        }
        .tipe-btn.active .tipe-icon { background:#ffedd5; }
        ...
        <div class="flex items-center gap-3">
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="text-sm font-semibold bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition">Keluar</button>
                </form>
            @else
        ...
        <form method="GET" action="{{ route('home') }}" id="search-form">
            ...
            <div class="flex gap-3 mb-4">
                <button type="button" onclick="setFilter('tipe_sewa','lepas_kunci')"
                    ...
                    <div class="tipe-icon">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="{{ $tipeSewa === 'lepas_kunci' ? '#c2410c' : '#6b7280' }}" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <button type="button" onclick="setFilter('tipe_sewa','dengan_sopir')"
                    ...
                    <div class="tipe-icon">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="{{ $tipeSewa === 'dengan_sopir' ? '#c2410c' : '#6b7280' }}" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
    ...
    @if($cars->isEmpty())
        <div class="flex flex-col items-center justify-center py-24 text-center">
            ...
            <a href="{{ route('home') }}" class="bg-brand bg-brand-hover text-white font-bold text-sm px-6 py-3 rounded-xl transition">
                Reset Filter
            </a>
        </div>
    ...
    @else
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
            @foreach($cars as $car)
                @php
                    // Gunakan harga_aktif yang sudah dihitung controller
                    $harga           = $car['harga_aktif'] ?? 0;
                    $hargaLepas      = $car['harga_lepas_kunci'];
                    $hargaSopir      = $car['harga_dengan_sopir'];
                ...
                <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden card-hover shadow-sm">
                    ...
                    <div class="relative bg-gray-50 aspect-[4/3] overflow-hidden">
                        ...
                        <span class="absolute bottom-3 right-3 text-xs font-bold bg-white/90 backdrop-blur px-2.5 py-1 rounded-full border border-gray-200 text-gray-600">
                            {{ $transmisi }}
                        </span>

                        <span class="absolute bottom-3 left-3 flex items-center gap-1 text-xs font-semibold bg-white/90 backdrop-blur px-2.5 py-1 rounded-full border border-gray-200 text-gray-600">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex gap-1.5 mb-3">
                            @if($bisaLepasKunci)
                                <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-orange-50 text-orange-600 border border-orange-200">
                                    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>
                                    Lepas kunci
                                </span>
                            @endif
                            @if($bisaDenganSopir)
                                <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-200">
