<x-filament-widgets::widget>
    <x-filament::section>
        ...
        <x-slot name="heading">
            <div style="display:flex;align-items:center;justify-content:space-between;width:100%;">
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        <p style="margin:0;font-size:14px;font-weight:700;color:var(--fi-color-gray-950,#111827);">Login Activity</p>
        ...
        <style>
            ...
            .la-stat:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(0,0,0,.1); }

            /* Light mode stat backgrounds */
            .la-stat.success { background:linear-gradient(160deg,#f0fdf4,#dcfce7); border-color:#86efac; }
            .la-stat.failed  { background:linear-gradient(160deg,#fff1f2,#fecdd3); border-color:#fca5a5; }
            .la-stat.users   { background:linear-gradient(160deg,#f5f3ff,#ede9fe); border-color:#c4b5fd; }
            ...
            .la-stat-bg-icon { position:absolute; right:-6px; bottom:-6px; opacity:.08; pointer-events:none; }

            .la-stat-top { display:flex; margin-bottom:14px; }
            .la-stat-icon-wrap {
                width:42px; height:42px; border-radius:12px;
                display:flex; align-items:center; justify-content:center; flex-shrink:0;
            }
            .la-stat.success .la-stat-icon-wrap { background:rgba(16,185,129,.15); }
            .la-stat.failed  .la-stat-icon-wrap { background:rgba(239,68,68,.12); }
            .la-stat.users   .la-stat-icon-wrap { background:rgba(124,58,237,.12); }

            .la-stat-val { font-size:32px; font-weight:900; line-height:1; letter-spacing:-.04em; }
            .la-stat.success .la-stat-val { color:#047857; }
            .la-stat.failed  .la-stat-val { color:#b91c1c; }
            .la-stat.users   .la-stat-val { color:#5b21b6; }
            .la-stat-label { font-size:11.5px; font-weight:600; margin-top:5px; color:#4b5563; }

            /* Dark: Filament adds .dark class to <html> */
            .dark .la-stat.success { background:linear-gradient(160deg,#052e16,#14532d); border-color:#166534; }
            .dark .la-stat.failed  { background:linear-gradient(160deg,#1a0505,#3f1d1d); border-color:#7f1d1d; }
            .dark .la-stat.users   { background:linear-gradient(160deg,#1e0a38,#2e1065); border-color:#4c1d95; }
            .dark .la-stat.success .la-stat-icon-wrap { background:rgba(16,185,129,.2); }
            .dark .la-stat.failed  .la-stat-icon-wrap { background:rgba(239,68,68,.2); }
            .dark .la-stat.users   .la-stat-icon-wrap { background:rgba(124,58,237,.2); }
            .dark .la-stat.success .la-stat-val { color:#4ade80; }
            .dark .la-stat.failed  .la-stat-val { color:#f87171; }
            .dark .la-stat.users   .la-stat-val { color:#c4b5fd; }
            .dark .la-stat-label   { color:#a8a29e; }
            ...
            .dark .la-filter-bar { background:#1c1917; border-color:#292524; }
            ...
            .dark .la-filter-group { background:#111110; border-color:#292524; box-shadow:none; }
            ...
            .dark .la-fbtn { color:#a8a29e; }
            .la-fbtn:hover { color:#111827; background:#f9fafb; }
            .dark .la-fbtn:hover { color:#e5e7eb; background:transparent; }
            .la-fbtn.fp-active  { background:linear-gradient(135deg,#7c3aed,#6d28d9); color:#fff !important; box-shadow:0 2px 8px rgba(124,58,237,.3); }
            .la-fbtn.fs-active-all     { background:#1f2937; color:#fff !important; }
            .la-fbtn.fs-active-success { background:#059669; color:#fff !important; }
            .la-fbtn.fs-active-failed  { background:#dc2626; color:#fff !important; }

            .la-filter-divider { width:1px; height:22px; background:#e5e7eb; margin:0 2px; flex-shrink:0; }
            .dark .la-filter-divider { background:#44403c; }
            ...
            .dark .la-count-badge { background:#292524; border-color:#44403c; color:#d4d0cb; }
            ...
            .dark .la-table-wrap { border-color:#292524; box-shadow:none; }
            .la-table { width:100%; border-collapse:collapse; }

            .la-thead-row { background:#f3f4f6; }
            .dark .la-thead-row { background:#1c1917; }
            ...
            .dark .la-th { color:#78716c; border-color:#292524; }

            .la-tr { border-bottom:1px solid #f3f4f6; transition:background .1s; }
            .dark .la-tr { border-color:#1c1917; }
            .la-tr:last-child { border-bottom:none; }
            .la-tr:hover { background:#f9fafb; }
            .dark .la-tr:hover { background:#111110; }
            ...
            .la-user-name  { font-size:13px; font-weight:700; color:#111827 !important; line-height:1.3; }
            .dark .la-user-name  { color:#f5f5f4 !important; }
            .la-user-email { font-size:11px; color:#6b7280 !important; margin-top:2px; }
            .dark .la-user-email { color:#78716c !important; }
            ...
            .la-status.success { background:#ecfdf5; color:#065f46 !important; border-color:#a7f3d0; }
            .la-status.failed  { background:#fef2f2; color:#991b1b !important; border-color:#fca5a5; }
            ...
            .dark .la-chip { background:#292524; color:#d4d0cb !important; border-color:#44403c; }
            .la-chip svg { opacity:.6; flex-shrink:0; }

            .la-time-rel { font-size:12px; font-weight:700; color:#111827 !important; }
            .dark .la-time-rel { color:#f5f5f4 !important; }
            .la-time-abs { font-size:10.5px; color:#9ca3af !important; margin-top:2px; }
            .dark .la-time-abs { color:#57534e !important; }

            .la-load-more {
                display:flex; justify-content:center; padding:14px;
                background:#f9fafb; border-top:1px solid #e5e7eb;
            }
            .dark .la-load-more { background:#1c1917; border-color:#292524; }
            .la-load-btn {
            ...
            .dark .la-load-btn { background:#292524; border-color:#44403c; color:#e5e7eb !important; }
            .la-load-btn:hover { background:#faf5ff; border-color:#c4b5fd; color:#5b21b6 !important; box-shadow:0 3px 10px rgba(124,58,237,.15); }
            .dark .la-load-btn:hover { background:#3c3836; border-color:#c4b5fd; color:#c4b5fd !important; }
            ...
            .dark .la-empty-icon { background:#1e0a38; border-color:#4c1d95; }
            .la-empty-title { font-size:14px; font-weight:700; color:#374151 !important; margin:0; }
            .dark .la-empty-title { color:#d4d0cb !important; }
            .la-empty-sub   { font-size:12px; color:#9ca3af !important; margin:5px 0 0; }
        ...
        <div class="la-stats">
            ...
            <div class="la-stat users">
                ...
                <div class="la-stat-top">
                    <div class="la-stat-icon-wrap">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="la-filter-bar">
            <div class="la-filter-group">
                @foreach(['today' => 'Hari Ini', 'week' => '7 Hari', 'month' => 'Bulan Ini'] as $key => $label)
                    <button wire:click="setPeriod('{{ $key }}')" class="la-fbtn {{ $filterPeriod === $key ? 'fp-active' : '' }}">{{ $label }}</button>
                @endforeach
            </div>
            <div class="la-filter-divider"></div>
            <div class="la-filter-group">
                @foreach(['all' => 'Semua', 'success' => 'Berhasil', 'failed' => 'Gagal'] as $key => $label)
                    <button wire:click="setFilter('{{ $key }}')" class="la-fbtn {{ $filterStatus === $key ? 'fs-active-'.$key : '' }}">{{ $label }}</button>
                @endforeach
            </div>
