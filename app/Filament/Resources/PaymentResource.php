    ...
    public static function table(Tables\Table $table): Tables\Table
    {
        return $table
            ...
            ->filters([
                ...
                Filter::make('tanggal_pembayaran')->form([
                    Forms\Components\Grid::make(2)->schema([
                        Forms\Components\Select::make('month')->label('Bulan')->options(array_reduce(range(1, 12), function ($carry, $month) {
                            $carry[$month] = Carbon::create(null, $month)->isoFormat('MMMM');
                            return $carry;
                        }, [])),
                        Forms\Components\Select::make('year')->label('Tahun')->options(function () {
                            $years = range(now()->year, now()->year - 5);
                            return array_combine($years, $years);
                        }),
                    ]),
                ])->query(function (Builder $query, array $data): Builder {
                    return $query->when($data['month'], fn(Builder $query, $month): Builder => $query->whereMonth('tanggal_pembayaran', $month))->when($data['year'], fn(Builder $query, $year): Builder => $query->whereYear('tanggal_pembayaran', $year));
                })->indicateUsing(function (array $data): ?string {
